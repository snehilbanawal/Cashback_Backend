...
def feedbackform(request):
    if request.method == 'POST':
        ...
        cont_obj = feedbackus(
            rating=ratingg, recommendation=recommendation1, service=service1, msg=message1)
        cont_obj.save()
...
def withdrawform2(request):
    ...
    if request.method == 'POST':
        name1 = request.POST['name1']
        user = Account.objects.get(email=name1)
        amount1 = user.cashback  # To make sure no 'hacking' takes place
